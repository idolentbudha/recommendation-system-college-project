...
public class updateServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            Long phoneno = Long.parseLong(request.getParameter("uphoneno"));
            String password0 = request.getParameter("upass0");
            ...
            PreparedStatement ps = con.prepareStatement("update users set userName=?, firstName = ?, password=?, email = ?, phonenumber = ? where userName = ? ");
            ps.setString(1, username);
            ps.setString(2, fullname);
            ps.setString(3, password0);
